...
export class TransactionProcessorService {
  ...
  private async processTransaction(tx: any, attempt: number) {
    ...
    try {
      ...
      // Step 6: Issue STAR rewards on-chain — wallet-aware minting.
      // Fetch pending rewards with the user's primary active wallet so we can
      // mint directly to their address; falls back to a DB-only record when
      // the user has no wallet. On-chain mint failure must never fail payment.
      ...
      for (const reward of pendingRewards) {
        ...
        if (walletAddress) {
          try {
            ...
            // Record for the completion adminLog (success path)
            rewardResult = { hash: mintResult.hash, starAmount: BigInt(reward.starAmount) }
          ...
          rewardResult = { hash: `db-only-no-wallet-${reward.id}`, starAmount: BigInt(reward.starAmount) }
        }
      }
