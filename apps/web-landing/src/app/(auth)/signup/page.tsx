  ...
  const onCreate = async () => {
    ...
    try {
      ...
      const payload: { email?: string; displayName: string } = {
        displayName: name || "PayEra user",
      };
      if (email) payload.email = email;
      const res = await sdk.auth.mockLogin(payload);
      setTokens(res.auth.accessToken, res.auth.refreshToken);
      if (res.user)
        setCurrentUser(res.user.id, res.user.displayName || name || "PayEra user");
      router.replace("/wallet");
    } catch (e: unknown) {
      setLocalErr(e instanceof Error ? e.message : "Could not create account");
    } finally {
      setBusy(false);
    }
  };
    ...
    <div className="w-full max-w-md">
      <div className="glass-strong rounded-3xl p-8">
        ...
        <p className="mt-2 text-sm text-fg-muted">
          The fastest way in is your Stellar wallet — your keys stay yours.
        </p>
        ...
        >
          ...
          {isConnecting ? "Connecting…" : "Continue with Freighter"}
        </button>
